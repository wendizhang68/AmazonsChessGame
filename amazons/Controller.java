...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("auto\\s+(?i)(white|black){1}", this::doAuto),
        new Command("manual\\s+(?i)(white|black){1}", this::doManual),
        new Command(String.format
                ("%s-%s\\(%s\\)", SQ, SQ, SQ), this::doMove),
        new Command(String.format("%s\\s+%s\\s+%s", SQ, SQ, SQ), this::doMove),
    };
     ...
     * Command "do Auto".
     */
    private void doAuto(Matcher piece) {
        String side = piece.group(1);
        if (side.charAt(0) == 'W' || side.charAt(0) == 'w') {
     ...
     * Command "do Manual".
     */
    private void doManual(Matcher piece) {
        String side = piece.group(1);
        if (side.charAt(0) == 'W' || side.charAt(0) == 'w') {
            _white = _manualPlayerTemplate.create(WHITE, this);
        } else {
